...
public class Student implements Comparable<Student> {
  ...
  @Override
    public String toString(){
      String b = (getLastName() + " " + getFirstName() + " " + (Arrays.toString(getHomeworkList().toArray()).replace('[', ' ').replace(']', ' ').replace(',', ' ').trim())
                  + " " + (Arrays.toString(getExamList().toArray()).replace('[', ' ').replace(']', ' ').replace(',', ' ').trim()));
      try {
        PrintWriter file = new PrintWriter(new File("gradebook.txt"));
        file.print(b);
        return b;
